...
# init_requests_session deprecated and removed
...
def refresh_clevertap_session(headed: bool = False, skip_logout: bool = False, manual: bool = False) -> None:
    ...
    try:
        ...
        if not skip_logout:
            ...
            try:
                page.goto("https://accounts.google.com/Logout", timeout=15000)
            except Exception as e:
                print(f"Google logout warning: {e}")
...
def get_todays_stats(campaign_id: int, today_str: str) -> dict[str, int | str]:
    global page_instance, HEADED_MODE
    
    print(f"  [BROWSER SCRAPER] Scraping stats for campaign {campaign_id}...")
    
    if not page_instance:
        refresh_clevertap_session(headed=HEADED_MODE, skip_logout=True)
        
    report_url = f"{CT_BASE_URL}/campaigns/campaign/{campaign_id}/report/stats"
    
    # We will attempt up to 2 times in case of session expiration / reload
    for attempt in range(2):
        try:
            print(f"    Navigating to stats URL: {report_url}")
            page_instance.goto(report_url, wait_until="domcontentloaded", timeout=30000)
            ...
            if "sso.clevertap.com" in curr_url or "google.com" in curr_url:
                print("    Session expired. Re-authenticating automatically...")
                refresh_clevertap_session(headed=HEADED_MODE, skip_logout=True)
                continue
                
            # Wait for stats cards to load and populate
            sent_count_locator = page_instance.locator('[data-testid="hero-card-sent-card"] .stats-card-count')
            sent_count_locator.wait_for(state="visible", timeout=15000)
            
            # Wait for text to be populated (not empty)
            for _ in range(50):
                val = sent_count_locator.inner_text().strip()
                if val:
                    break
                page_instance.wait_for_timeout(100)
            
            # Scrape Sent
            sent_text = page_instance.locator('[data-testid="hero-card-sent-card"] .stats-card-count').inner_text().strip()
            
            # Scrape Delivered
            del_card = page_instance.locator('[data-testid="hero-card-delivered-card"]')
            del_sec = del_card.locator('.stats-card-secondary-count')
            if del_sec.is_visible():
                del_text = del_sec.inner_text().strip()
            else:
                del_text = del_card.locator('.stats-card-count').inner_text().strip()
                
            # Scrape Viewed
            view_card = page_instance.locator('[data-testid="hero-card-viewed-card"]')
            view_sec = view_card.locator('.stats-card-secondary-count')
            if view_sec.is_visible():
                view_text = view_sec.inner_text().strip()
            else:
                view_text = view_card.locator('.stats-card-count').inner_text().strip()
                
            # Scrape Clicks
            clicks_card = page_instance.locator('[data-testid="hero-card-clicks-card"]')
            clicks_sec = clicks_card.locator('.stats-card-secondary-count')
            if clicks_sec.is_visible():
                clicks_text = clicks_sec.inner_text().strip()
            else:
                clicks_text = clicks_card.locator('.stats-card-count').inner_text().strip()
                
            # Clean and parse helper
            def parse_val(text: str) -> int | str:
            ...
            sent_val = parse_val(sent_text)
            del_val = parse_val(del_text)
            view_val = parse_val(view_text)
            click_val = parse_val(clicks_text)
            
            print(f"    [BROWSER SCRAPER] Successfully scraped: Sent={sent_val} | Delivered={del_val} | Viewed={view_val} | Clicked={click_val}")
            ...
            print(f"    Warning: Browser scraping attempt {attempt + 1} failed: {e}")
            if attempt == 0:
                print("    Retrying navigation after potential session refresh...")
                try:
                    refresh_clevertap_session(headed=HEADED_MODE, skip_logout=True)
                except Exception:
                    pass
    ...
    print(f"  [BROWSER SCRAPER] Error: Failed to scrape campaign {campaign_id} via DOM.")
    return {
        "sent": "ERROR",
        "delivered": "ERROR",
        "viewed": "ERROR",
...
def scrape_journey_tab_from_browser(tab_name: str, campaigns: list[tuple[str, int]], run_date: str) -> dict[int, dict[str, int]]:
    ...
    try:
        ...
        if journey_row:
            ...
            if href:
                import re
                m = re.search(r'/journey/(\d+)', href)
                if m:
                    journey_id = m.group(1)
                    canvas_url = f"{CT_BASE_URL}/journeys/journey/{journey_id}/report/node-stats"
                    print(f"[VISUAL SCRAPER] Extracted Journey ID {journey_id}. Navigating directly to canvas: {canvas_url}")
            if not canvas_url:
            ...
            if link.is_visible():
                ...
                if href:
                    ...
                    if m:
                        journey_id = m.group(1)
                        canvas_url = f"{CT_BASE_URL}/journeys/journey/{journey_id}/report/node-stats"
                        print(f"[VISUAL SCRAPER] Extracted Journey ID {journey_id} from fallback. Navigating directly to canvas: {canvas_url}")
                if not canvas_url:
                    print(f"[VISUAL SCRAPER] Clicking fallback journey link containing: {tab_name}")
                    link.click()
                    page_instance.wait_for_timeout(5000)
            else:
                print(f"[VISUAL SCRAPER] Warning: Journey for {tab_name} not found in the list.")
                return stats_map
                
        if canvas_url:
            page_instance.goto(canvas_url, wait_until="domcontentloaded", timeout=20000)
            page_instance.wait_for_timeout(5000)
        else:
        ...
        for campaign_name, campaign_id in campaigns:
            try:
                ...
                # Check different name candidates on canvas
                node_candidates = [
                    campaign_name,
                    campaign_name.replace("_", " "),
                    campaign_name.replace("WA_", "").replace("_", " "),
                    campaign_name.replace("WA_", "").replace("_", " ").replace("Day", " Day"),
                ]
                
                node = None
                for candidate in node_candidates:
                    loc = page_instance.get_by_text(candidate).last
                    try:
                        if loc.count() > 0:
                            node = loc
                            print(f"    Found node in canvas DOM using candidate name: '{candidate}'")
                            break
                    except Exception:
                        pass
                
                if not node:
                    raise Exception(f"Node element not found on canvas for candidates: {node_candidates}")
                ...
                stats_tab = None
                stats_btn_locator = page_instance.locator('text="Stats"').last
                stats_btn_locator.wait_for(state="visible", timeout=10000)
                ...
                # Wait for metric value to be populated
                sent_lbl_locator = page_instance.locator('div:has-text("Sent")').last
                sent_lbl_locator.wait_for(state="visible", timeout=10000)
                
                for _ in range(50):
                    if sent_lbl_locator.inner_text().strip():
                        break
                    page_instance.wait_for_timeout(100)
                ...
                clicks_locator = page_instance.locator('div:has-text("Clicks"), div:has-text("Clicked")').last
                clicks_text = clicks_locator.inner_text()
                ...
                # Close modal and verify it is closed
                done_btn = page_instance.locator('button:has-text("Done"), span:has-text("Done")').first
                done_btn.click()
                done_btn.wait_for(state="hidden", timeout=5000)
                page_instance.wait_for_timeout(1000)
            except Exception as e:
                print(f"    Warning: Failed to visually scrape node {campaign_name}: {e}.")
                # Attempt to close modal in case it was left open
                try:
                    done_btn = page_instance.locator('button:has-text("Done"), span:has-text("Done")').first
                    done_btn.click()
                    done_btn.wait_for(state="hidden", timeout=2000)
                except Exception:
                    pass
...
def write_daily_tab(sheet: gspread.Spreadsheet, tab_name: str, campaigns: list[tuple[str, int]], run_date: str) -> None:
    ...
    for node_name, campaign_id in campaigns:
        if campaign_id in scraped_stats:
            ...
            print(f"  Fetching {node_name} (Campaign ID: {campaign_id}) via browser standalone report fallback...")
    ...
    if any("ERROR" in row[3:] for row in rows):
        print(f"  Skipped writing to '{tab_name}' because one or more stats extraction tasks failed\n")
        return
